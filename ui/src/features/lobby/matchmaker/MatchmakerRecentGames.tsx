...
// replaces whatever the Replays tab was showing.
  ...
  return (
    <section className="matchmaker-card surface-panel matchmaker-recent" aria-labelledby="matchmaker-recent-title">
      ...
      {status.type === "failed" ? (
        ...
        <ol className="matchmaker-recent-list">
          {games.map((game) => {
            ...
            return (
              <li key={game.uid} className="matchmaker-recent-row">
                <span className="matchmaker-recent-mode">{side.mode}</span>
                <span className="matchmaker-recent-game">
                  <strong>{mapPresentation(vault, game.map).displayName}</strong>
                  <small className="muted">
                    {side.opponents.length > 0
                      ? t("lobby.matchmaker.recent.versus", { players: side.opponents.join(", ") })
                      : formatAgeOrDate(game.startTime)}
                  </small>
                </span>
                <span className="muted matchmaker-recent-when">{formatAgeOrDate(game.startTime)}</span>
                <span className={`matchmaker-recent-outcome${outcome ? ` is-${outcome}` : ""}`}>
                  {outcomeLabel(side.me?.outcome ?? "") || t("lobby.matchmaker.recent.noResult")}
                </span>
                <span
                  className={`matchmaker-recent-change${change === null ? "" : change >= 0 ? " is-up" : " is-down"}`}
                  title={t("lobby.matchmaker.recent.ratingChange")}
                >
