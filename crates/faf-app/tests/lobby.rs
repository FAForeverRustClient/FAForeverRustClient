...
use faf_app::App;
use faf_domain::state::{GameVisibility, HostGameRequest, HostState, JoinState, LobbyCommand, LobbyStatus};
...
#[tokio::test]
async fn host_emits_hosting_then_hosted() {
    let (app, app_loop) = App::new("test", fake_ports());
    tokio::spawn(app_loop.run());
    let mut events = app.subscribe();
    ...
    // Wait until the connection is live before hosting, so the fake has stored
    // the live update sender.
    loop {
        if let AppEvent::Lobby(faf_domain::state::LobbyEvent::GamesUpdated { .. }) =
            events.recv().await.unwrap()
        {
    ...
    // Optimistic Hosting is emitted immediately by the service.
    loop {
        match events.recv().await.unwrap() {
            AppEvent::Lobby(faf_domain::state::LobbyEvent::Hosting) => break,
            _ => continue, // a GamesUpdated tick may interleave
        }
    }

    // The fake replies with a Hosted id on the same stream.
    loop {
        if let AppEvent::Lobby(faf_domain::state::LobbyEvent::Hosted { .. }) =
            events.recv().await.unwrap()
        {
            break;
        }
    ...
    let snap = app.snapshot();
    assert!(matches!(snap.lobby.host, HostState::Hosted { .. }));
}
