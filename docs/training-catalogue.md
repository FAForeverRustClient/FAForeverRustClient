...
`faf-bo/1` envelope the BO recorder's `bo2json` produces:

```json
{
  "id": "sladow-1v1-twin-rivers",
  "recordingUrl": "https://raw.githubusercontent.com/FAForeverRustClient/guides/main/runs/sladow-1v1-twin-rivers.json"
}
```

The client then draws the entry as three panes rather than one: the written
order, what happened and when, and the ground it happened on. Pointing at a
unit in one pane highlights it in the other.

The same trust rule as a guide applies, and for the same reason: an address
outside the repository this build is configured to trust is dropped where the
manifest is parsed, so it never becomes a request. Raw address again, because
the client reads the document itself.

**Nothing states this field yet.** The renderer ships ahead of the data: a run
has to be played with the recorder to exist at all, and no catalogued build
order has been. An entry without the field is unaffected and draws exactly as
it did before.

Two things the envelope decides, worth knowing before recording:

- The map pane needs `meta.map.sizeX` and `sizeZ`. A run recorded before those
  were written says so instead of drawing an empty square.
- Positions are already on the heightmap scale, so the client places them over
